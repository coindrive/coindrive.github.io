...
angular.module('myApp.controllers', ['myApp.i18n'])
  ...
  .controller('AppLoginController', function ($window, $scope, $rootScope, $location, $timeout, $modal, $modalStack, MtpApiManager, ErrorService, NotificationsManager, PasswordManager, ChangelogNotifyService, IdleManager, LayoutSwitchService, TelegramMeWebService, _) {
	delete window.localStorage.removeItem("bitGoId");
	$rootScope.user = {username:"" ,password:"",otp:""}
	if(!$rootScope.Backendless){
		$rootScope.Backendless = $window.Backendless;
		$rootScope.Backendless.initApp('71500CE3-CA63-4E67-FFF9-B534046BEF00', '2CDF530E-1B1E-1835-FF2D-49FB1552A400', 'v1' );
	}
	function regBackend(){
		var user = new Backendless.User();
		user.email = $scope.credentials.email;
    ...
    $scope.credentials = {phone_country: '', phone_country_name: '', phone_number: '', phone_full: '',email: '', password:'',bitgo_code:''};
    $scope.progress = {};
    $scope.callPending = {};
    $scope.about = {};
	$rootScope.linkBetween = $scope.credentials;
...
// BITGOJS!!!!!
	...
	$rootScope.user.username = $scope.credentials.email
	$rootScope.user.password = $scope.credentials.password
	$scope.loginBackend()
	try{
  ...
  .controller('AppIMController', function ($cacheFactory,$window,$interval,AcceptShareWalletService,$q, qSync,SendCoinChatWindowService, $scope, $location, $routeParams, $modal, $rootScope, $modalStack, MtpApiManager, AppUsersManager, AppChatsManager, AppMessagesManager, AppPeersManager, ContactsSelectService, ChangelogNotifyService, CreateWalletService, ErrorService, AppRuntimeManager, HttpsMigrateService, LayoutSwitchService, LocationParamsService, AppStickersManager) {
	if(!$rootScope.Backendless){
		$rootScope.Backendless = $window.Backendless;
		$rootScope.Backendless.initApp('71500CE3-CA63-4E67-FFF9-B534046BEF00', '2CDF530E-1B1E-1835-FF2D-49FB1552A400', 'v1' );
	}
	if( $rootScope.Backendless.LocalCache.get("user-token") );
	$rootScope.props = Backendless.UserService.describeUserClass();
	$scope.authBitgo = $interval(bitgoCache,1000);
	function bitgoCache () {
	var cached = {};
	$rootScope.bitcoinUserID;
		if ($rootScope.bitgo) {
			if(!$rootScope.bitgo._user) return;
			cached = {
				_clientId: $rootScope.bitgo._clientId,
				_clientSecret: $rootScope.bitgo._clientSecret,
				_keychains: $rootScope.bitgo._keychains,
				_id:$rootScope.bitgo._user.id,
				_refreshToken: $rootScope.bitgo._refreshToken,
				_token: $rootScope.bitgo._token}

			cached = JSON.stringify(cached);
			$window.localStorage['bitGoId'] = cached;
			$interval.cancel($scope.authBitgo);
			
		} else {
			$rootScope.bitgo = $window.bitgo;
			cached = $window.localStorage.bitGoId;
			var stepa = angular.fromJson(cached);
			$rootScope.bitcoinUserID = stepa._id;
			$rootScope.bitgo._clientId = stepa._clientId;
			$rootScope.bitgo._clientSecret = stepa._clientSecret;
			$rootScope.bitgo._keychains = stepa._keychains;
			$rootScope.bitgo._refreshToken = stepa._refreshToken;
			$rootScope.bitgo._token = stepa._token;
			$interval.cancel($scope.authBitgo)
		} 
		$rootScope.bitgo.wallets().listShares({}, function callback(err, result) {
			if (err) {
			  throw err;
			}
			$rootScope.listShareWallets = result
			for(var i =0;i < $rootScope.listShareWallets.incoming.length;i++){
				for(var j = 0; j < $rootScope.BLTable.data.length;j++)
				if($rootScope.listShareWallets.incoming[i].fromUser === $rootScope.BLTable.data[j].idUserBitcoin){
					$rootScope.listShareWallets.incoming[i].lastName = $rootScope.BLTable.data[j].lastName;
					$rootScope.listShareWallets.incoming[i].firstName = $rootScope.BLTable.data[j].firstName;
					$rootScope.listShareWallets.incoming[i].email = $rootScope.BLTable.data[j].email;
					$rootScope.listShareWallets.incoming[i].phoneNumber = $rootScope.BLTable.data[j].phoneNumber;
				}
			}
		});
	}
	$scope.$on('$routeUpdate', updateCurDialog);
	$scope.sendCoinChatWindow = function(){
		SendCoinChatWindowService.sendCoinChatWindow()
	}
	$scope.acceptWallet = function(){
		AcceptShareWalletService.AcceptShareWallet()
	}
  ...
  .controller('AppImDialogsController', function ($scope,$rootScope, $location, $q, $timeout, $routeParams, MtpApiManager, AppUsersManager, AppChatsManager, AppMessagesManager, AppProfileManager, AppPeersManager, PhonebookContactsService, ErrorService, AppRuntimeManager) {
    $scope.dialogs = [];
    $scope.contacts = [];
    $scope.foundPeers = [];
    $scope.foundMessages = [];
    ...
    function loadDialogs (force) {
      ...
      getDialogs(force).then(function (dialogsResult) {
		...
		$rootScope.ChatUserSelf = AppUsersManager.getSelf()
		$rootScope.BLTable = $rootScope.Backendless.Persistence.of("CoinDriveWithWeb").find()
			for (var i = 0; i < $rootScope.BLTable.data.length;i++){
				if($rootScope.BLTable.data[i].phoneNumber === $rootScope.ChatUserSelf.phone){
					if(!$rootScope.BLTable.data[i].idUserChat){
						$rootScope.BLTable.data[i].firstName = $rootScope.ChatUserSelf.first_name;
						$rootScope.BLTable.data[i].lastName = $rootScope.ChatUserSelf.last_name;
						$rootScope.BLTable.data[i].idUserChat = $rootScope.ChatUserSelf.id +'';
						$rootScope.BLTable.data[i].userName = $rootScope.ChatUserSelf.username;
						$rootScope.BLTable.data[i].idUserBitcoin = $rootScope.bitgo._user.id;
						$rootScope.BLTable.data[i].photo_id = $rootScope.ChatUserSelf.photo.photo_id;
						var obj = $rootScope.BLTable.data[i]
						$rootScope.Backendless.Persistence.of("CoinDriveWithWeb").save(obj)
					}
					break;
				} else if (i == $rootScope.BLTable.data.length - 1 && $rootScope.linkBetween.email != $rootScope.BLTable.data[i].email){
						var obj = {};
						obj.idUserBitcoin = $rootScope.bitgo._user.id
						obj.email = $rootScope.linkBetween.email;
						obj.phoneNumber = $rootScope.ChatUserSelf.phone;
						obj.firstName = $rootScope.ChatUserSelf.first_name;
						obj.lastName = $rootScope.ChatUserSelf.last_name;
						obj.idUserChat = $rootScope.ChatUserSelf.id +'';
						obj.userName = $rootScope.ChatUserSelf.username;
						$rootScope.Backendless.Persistence.of("CoinDriveWithWeb").save(obj);
				}
			}
  ...
  .controller('ContactsModalController', function ($scope,$rootScope, $timeout, $modal,SendCoinContactWindowService, $modalInstance, MtpApiManager,AddUserContactService, AppUsersManager, ErrorService) {
	
	$scope.sendCoinContactWindow = function(){
		SendCoinContactWindowService.sendCoinContactWindow()
	}
	$rootScope.contactUserID='Hello';
	$scope.setContactUserID = function(id){
		$rootScope.contactUserID = id+""
	}
	$scope.openAddUser = function () {
      AddUserContactService.addUserContactModal(false);
    }
    $scope.contacts = [];
    $scope.foundPeers = [];
    $scope.search = {};
...
.controller("WalletController", function($scope,$rootScope,CreateWalletService,toastr,AppPhotosManager,ApiUpdatesManager,AppUsersManager,MtpApiManager,MtpApiFileManager, $interval,$window,$timeout,$q,AddUserService,SendCoinService){
	...
	MtpApiManager.invokeApi('users.getFullUser', {
      ...
      AppPhotosManager.savePhoto(userFullResult.profile_photo, {
		...
		if( Backendless.LocalCache.get("user-token") );
			var st = Backendless.Persistence.of("CoinDriveWithWeb").find()
			function bitgoCache () {
			var cached = {};
			if ($rootScope.bitgo) {
				if(!$rootScope.bitgo._user) return;
	...
	$rootScope.newWallet = {passphrase: "",
							label: "",
							backupXpubProvider: "keyternal"};
	$scope.$watch($rootScope.label);
	$scope.$watch($rootScope.passphrase);
   	$scope.openCreateWallet = function () {
      CreateWalletService.createWallet(false);
    }
	
	$scope.openAddUser = function () {
      AddUserService.addUserModal(false);
    }
	$rootScope.listWallets;	
	$scope.idWalletChatRoom;
    $rootScope.getListWallets = function(){
		$scope.walletlabels;
		var wallets = $rootScope.bitgo.wallets();
		wallets.list({}, function callback(err, wallets) {
			$rootScope.listWallets = wallets.wallets;
			for (var i = 0; i < $rootScope.listWallets.length ;i++){
				$scope.getBalance($rootScope.listWallets[i].wallet.id,i) 
			} 
			if(!$rootScope.focusedWallet){
			$rootScope.focusedWallet = $rootScope.listWallets[0];
			}
		})
	}
	$scope.lastBalance = 0;
	$scope.getBalance = function(id,index){
		...
		wallets.get(data, function callback(err, wallet) {
			...
			$rootScope.listWallets[index].wallet.balance = wallet.balance();
			wallet.transactions({}, function callback(err, transactions) {
				$rootScope.listWallets[index].wallet.transactions = transactions.transactions;
				for (var i = $rootScope.listWallets[index].wallet.transactions.length - 1; i >=0 ; i--){
					for(var j = 0; j < $rootScope.listWallets[index].wallet.transactions[i].entries.length; j++){
						if($rootScope.listWallets[index].wallet.transactions[i].entries[j].account === $rootScope.listWallets[index].wallet.id){
							$scope.lastBalance += $rootScope.listWallets[index].wallet.transactions[i].entries[j].value;
							$rootScope.listWallets[index].wallet.transactions[i].entries[j].lastBalance = $scope.lastBalance;
						}
					}
				}
				$scope.lastBalance = 0;
			...
			wallet.addresses({}, function(err, walletAddress) {
				$rootScope.listWallets[index].wallet.listAddresses = walletAddress;
				$rootScope.bitgo.labels({}, function callback(err, labels) {
				if (err) {
				
				}	
					var listLabelForWallet = [];
					for(var i = 0; i < $rootScope.listWallets[index].wallet.listAddresses.addresses.length;i++){
						for(var j = 0;j < labels.length;j++){
							if($rootScope.listWallets[index].wallet.listAddresses.addresses[i].address === labels[j].address){
								$rootScope.listWallets[index].wallet.listAddresses.addresses[i].label = labels[j].label
								listLabelForWallet.push(labels[j].label)
							} else {
								$rootScope.listWallets[index].wallet.listAddresses.addresses[i].label = 'No address label.';
							}
						}
					}
					if(listLabelForWallet.length != 0){
						$rootScope.listWallets[index].wallet.label = listLabelForWallet[listLabelForWallet.length - 1];
					}						
				});
				
			});
				if(wallet.wallet.admin){
				$rootScope.listWallets[index].wallet.users = wallet.wallet.admin.users;
				} else {
					$rootScope.listWallets[index].wallet.users = false;
				}
				for(var i = 0;i < $rootScope.listWallets[index].wallet.users.length;i++){
					for(var j = 0;j < $rootScope.BLTable.data.length;j++){
						if($rootScope.listWallets[index].wallet.users[i].user === $rootScope.BLTable.data[j].idUserBitcoin){
							$rootScope.listWallets[index].wallet.users[i].firstName = $rootScope.BLTable.data[j].firstName;
							$rootScope.listWallets[index].wallet.users[i].lastName = $rootScope.BLTable.data[j].lastName;
							$rootScope.listWallets[index].wallet.users[i].phoneNumber = $rootScope.BLTable.data[j].phoneNumber;
	...
	$scope.createWallet = function (){
		$rootScope.newWallet.label = $scope.label;
		$rootScope.newWallet.passphrase = $scope.passphrase;
		$rootScope.bitgo.wallets().createWalletWithKeychains($rootScope.newWallet,  function(err, result) {
			if (err) { console.dir(err); throw new Error("Could not create wallet!"); }
			toastr.success("Wallet was created!","Success")
	...
	$scope.walletTab = 1;
	$rootScope.newUserObj = { 
							email: '',
							permissions: '',
							skipKeychain: true,
							disableEmail: true
						}
	$scope.addUserInWallet = function(){
		var idWallet = $rootScope.focusedWallet.wallet.id
		if($scope.idWalletChatRoom){
			idWallet = $scope.idWalletChatRoom
		}
		bitgo.wallets().get({ "id": idWallet }, function(err, wallet) {
			if($rootScope.contactUserID){
				for(var i = 0; i < $rootScope.BLTable.data.length;i++){
					if($rootScope.contactUserID == $rootScope.BLTable.data[i].idUserChat){
						$rootScope.newUserObj.email = $rootScope.BLTable.data[i].email;
					}
				}
				
			}
			wallet.shareWallet($rootScope.newUserObj, function callback(err, result) {
				if(err){
	...
	$scope.$on("UpdateWallet",function(){
		$rootScope.getListWallets()
	...
	$scope.acceptShareWallet = function () {
		$rootScope.bitgo.wallets().acceptShare($scope.shareAttr, function(err, result) {
				if(err){
					toastr.error('Share error')
				} else {
					toastr.success('You accepted share wallet','Success')
					$rootScope.$broadcast('UpdateWallet');
				}
	...
	$scope.refuseShareWallet = function(shareId){
		var id = $scope.shareAttr.walletShareId;
		if (shareId){
			id = shareId;
		}
		$rootScope.bitgo.wallets().cancelShare({walletShareId: id}, function(err, result) {
			if (err) { throw err; }
			toastr.info('You refuse from share wallet')
		})
	}
	$rootScope.BLTable = $rootScope.Backendless.Persistence.of("CoinDriveWithWeb").find();
	$rootScope.bitgo.wallets().listShares({}, function callback(err, result) {
		...
		for(var i =0;i < $rootScope.listShareWallets.outgoing.length;i++){
			for(var j = 0; j < $rootScope.BLTable.data.length;j++)
			if($rootScope.listShareWallets.outgoing[i].toUser === $rootScope.BLTable.data[j].idUserBitcoin){
				$rootScope.listShareWallets.outgoing[i].lastName = $rootScope.BLTable.data[j].lastName;
				$rootScope.listShareWallets.outgoing[i].firstName = $rootScope.BLTable.data[j].firstName;
				$rootScope.listShareWallets.outgoing[i].email = $rootScope.BLTable.data[j].email;
				$rootScope.listShareWallets.outgoing[i].phoneNumber = $rootScope.BLTable.data[j].phoneNumber;
				$rootScope.listShareWallets.outgoing[i].idUserChat = $rootScope.BLTable.data[j].idUserChat;
			}
		}
		$scope.outgoingShare = {}
		for(var i =0;i < $rootScope.listShareWallets.outgoing.length;i++){
			$scope.outgoingShare[$rootScope.listShareWallets.outgoing[i].walletId] = {
				lastName:$rootScope.listShareWallets.outgoing[i].lastName,
				firstName:$rootScope.listShareWallets.outgoing[i].firstName,
				email:$rootScope.listShareWallets.outgoing[i].email,
				phoneNumber:$rootScope.listShareWallets.outgoing[i].phoneNumber,
				idUserChat:$rootScope.listShareWallets.outgoing[i].idUserChat,
				permissions:$rootScope.listShareWallets.outgoing[i].permissions,
				id:$rootScope.listShareWallets.outgoing[i].id,
				state:$rootScope.listShareWallets.outgoing[i].state,
	...
	$scope.freezeWallet = function(){
		$rootScope.bitgo.wallets().get({ "id": $rootScope.focusedWallet.wallet.id }, function(err, wallet) {
			bitgo.unlock($scope.smsCode, function(err) {
			if (err) {
			console.dir(err);
			throw new Error("Could not unlock!");
			}
				wallet.freeze({"duration":+$scope.duration.duration}, function(err, result) {
					if(err){
						toastr.error('Freeze wallet error')
					} else {
						toastr.success('You freeze your wallet','Success')
						$rootScope.$broadcast('UpdateWallet');
					}
	  });
	});
	...
	$rootScope.BLTable = $rootScope.Backendless.Persistence.of("CoinDriveWithWeb").find()
	for (var i = 0; i < $rootScope.BLTable.data.length;i++){
				if($rootScope.BLTable.data[i].idUserBitcoin === $rootScope.bitcoinUserID){
					$rootScope.meOnBL = $rootScope.BLTable.data[i];
					break;
				} 
			}
	$scope.selectDefaultWallet = function (){
			for (var i = 0; i < $rootScope.BLTable.data.length;i++){
				if($rootScope.BLTable.data[i].idUserBitcoin === $rootScope.bitcoinUserID){
					$rootScope.BLTable.data[i].defaultWalletID = $rootScope.focusedWallet.wallet.id
					var obj = $rootScope.BLTable.data[i]
					$rootScope.Backendless.Persistence.of("CoinDriveWithWeb").save(obj)
					$rootScope.BLTable = $rootScope.Backendless.Persistence.of("CoinDriveWithWeb").find();
					$rootScope.meOnBL = $rootScope.BLTable.data[i];
	...
	$scope.sendBitcoinsContactWindow = function(){
		...
		if($scope.useDefaultWallet){
			for(var i = 0;i < $rootScope.BLTable.data.length;i++){
					if($rootScope.contactUserID === $rootScope.BLTable.data[i].idUserChat){			
						$scope.attrTransactions.address = $rootScope.BLTable.data[i].defaultWalletID;
					}
			}
		}
		...
		$rootScope.bitgo.wallets().get({id: $scope.idWalletChatRoom}, function(err, wallet) {
			if (err) { console.log("Error getting wallet!"); console.dir(err); return process.exit(-1); }
			 $rootScope.bitgo.unlock($scope.smsCode, function(err) {
				if (err) {
				  console.dir(err);
				  throw new Error("Could not unlock!");
				}
				wallet.sendCoins($scope.attrTransactions, function(err, result) {
	...
	$scope.newTransactions = function(){
		SendCoinService.sendCoin()
	}
	$scope.newAddressLabel = {
		label:''
	};
	$scope.$watch($scope.newAddressLabel)
	$scope.tabAddresses = false;
	$scope.changeWalletName = function(){
		var wallets = $rootScope.bitgo.wallets();
		wallets.get({"type": "bitcoin","id":$rootScope.focusedWallet.wallet.id}, function callback(err, wallet) {
			...
			wallet.setLabel($scope.newAddressLabel, function (err, label) {
				if (err) { toastr.error("Error sending coins!"); return process.exit(-1); }
					toastr.success('Label was created!','Success')
					$rootScope.$broadcast('UpdateWallet');
				});
			});
		};
	...
	$scope.removeAddressLabel = function (){
		...
		wallet.deleteLabel({address: $scope.newAddressLabel.address}, function (err, label) {
	  if (err) {
		toastr.error("Can`t delete label")
	  }
	  toastr.success('Address lebel was removed', 'Success!');
	  $rootScope.$broadcast('UpdateWallet');
	});
	});
	}
